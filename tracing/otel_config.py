            ...
            with tracer.start_as_current_span(span_name) as span:
                ...
                try:
                    ...
                    if _metrics is not None:
                        _metrics.record_call(agent_name, duration_ms, True)
                    ...
                    if _metrics is not None:
                        _metrics.record_call(agent_name, duration_ms, False)
...
def set_agent_metrics(metrics: AgentMetrics) -> None:
    """注册全局 Agent 指标收集器，供 trace 装饰器写入 /api/metrics。"""
    global _metrics
    _metrics = metrics


def annotate_llm_token_usage(response: Any) -> None:
    """将 LangChain AIMessage 的 token_usage 写入当前 Span。"""
    if not _HAS_OTEL:
        return
    ...
    if prompt is not None:
        span.set_attribute("llm.prompt_tokens", int(prompt))
    if completion is not None:
        span.set_attribute("llm.completion_tokens", int(completion))
    if total is not None:
        span.set_attribute("llm.total_tokens", int(total))
...
def create_traced_chat_openai(**kwargs: Any) -> Any:
    ...
    class TracedChatOpenAI(ChatOpenAI):
        async def ainvoke(self, input, config=None, **kw):
            resp = await super().ainvoke(input, config=config, **kw)
            annotate_llm_token_usage(resp)
            return resp

    return TracedChatOpenAI(**kwargs)
